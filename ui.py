...
class MainWindow:
	...
	def move(self, how):
		...
		self.jinx.position = {
			...
			"pgup":  self.jinx.position - self.width * int(height * 2 // 3),
			"pgdn":  self.jinx.position + self.width * int(height * 2 // 3),
			"home":  int(self.jinx.position // self.width) * self.width,
			"end":   int(self.jinx.position // self.width) * (self.width+1) - 1,
			"top":   0,
